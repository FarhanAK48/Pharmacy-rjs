import React, { useEffect, useState } from 'react'
 ...
 const AddProduct = ({addProduct, showAddProduct, editMedicine, clearEditMedicine }) => {
 ...
 const submitData = (e) => {
    ...
    if(formData.name && formData.desc && formData.type && formData.price && formData.expDate && formData.totalItem && formData.quantity){
      if(editMedicine){
        addProduct(editMedicine.id, formData.name, formData.desc, formData.type, formData.price, formData.expDate, formData.totalItem,formData.quantity)
        const exists = medicine.find((item) => item.name.toLowerCase() === formData.name.toLowerCase())
        if(exists){
            formData.name = "";
            formData.desc = "";
            formData.type= "";
        ...
        console.log('ID', id)
        addProduct(id, formData.name, formData.desc, formData.type, formData.price, formData.expDate, formData.totalItem,formData.quantity)
       const exists = medicine.find((item) => item.name.toLowerCase() === formData.name.toLowerCase())
      //  if(!editMedicine && !exists){
           formData.name = "";
           formData.desc = "";
           formData.type= "";
           formData.quantity= 1
 ...
 const onCancel = () => {
  ...
  return (
    ...
    <div className="mb-6">
      <h2 className='text-2xl font-bold text-gray-600 mb-4'>Add New Product</h2>
